...
export async function GET(req: any) {
  ...
  try {
    ...
    const [elimina] = await conexion.query(
      `DELETE newProcesosEvaluacion.*, newProcesoEstudiante.*, newBancoProcesos.* FROM newProcesosEvaluacion LEFT JOIN newProcesoEstudiante ON newProcesosEvaluacion.id = newProcesoEstudiante.id INNER JOIN newBancoProcesos ON newBancoProcesos.id = newProcesosEvaluacion.relacionBanco WHERE newProcesosEvaluacion.id = ${id}`
    );
    console.log(
      `DELETE newProcesosEvaluacion.*, newProcesoEstudiante.*, newBancoProcesos.* FROM newProcesosEvaluacion LEFT JOIN newProcesoEstudiante ON newProcesosEvaluacion.id = newProcesoEstudiante.id INNER JOIN newBancoProcesos ON newBancoProcesos.id = newProcesosEvaluacion.relacionBanco WHERE newProcesosEvaluacion.id = ${id}`
    );
